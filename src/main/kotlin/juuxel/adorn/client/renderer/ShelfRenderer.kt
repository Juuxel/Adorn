...
class ShelfRenderer(dispatcher: BlockEntityRenderDispatcher) : BlockEntityRenderer<ShelfBlockEntity>(dispatcher) {
    override fun render(
        ...
        matrices.multiply(Vector3f.POSITIVE_Y.getDegreesQuaternion(ITEM_1_Y_ROT + 180 - facing.asRotation()))
        itemRenderer.renderItem(be.getStack(0), ModelTransformation.Mode.FIXED, light, overlay, matrices, vertexConsumerProvider)
        matrices.pop()

        matrices.push()
        matrices.translate(tx2, 9.6 / 16.0, tz2)
        matrices.scale(ITEM_SCALE, ITEM_SCALE, ITEM_SCALE)
        matrices.multiply(Vector3f.POSITIVE_Y.getDegreesQuaternion(ITEM_2_Y_ROT + 180 - facing.asRotation()))
        itemRenderer.renderItem(be.getStack(1), ModelTransformation.Mode.FIXED, light, overlay, matrices, vertexConsumerProvider)
        matrices.pop()
    }
