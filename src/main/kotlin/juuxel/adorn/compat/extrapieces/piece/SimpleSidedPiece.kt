...
abstract class SimpleSidedPiece(id: Identifier, private val modelTemplate: Identifier) : AdornPiece(id) {
    override fun addBlockModels(pack: ArtificeResourcePack.ClientResourcePackBuilder, pb: PieceBlock) {
        ...
        pack.addBlockModel(id) {
            ...
            it.texture("side", pb.set.mainTexture)
            it.texture("up", pb.set.topTexture)
            it.texture("down", pb.set.bottomTexture)
        }
    }
}
