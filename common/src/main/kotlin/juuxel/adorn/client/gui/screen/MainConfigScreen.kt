...
class MainConfigScreen(parent: Screen) : AbstractConfigScreen(Text.translatable("gui.adorn.config.title"), parent) {
    override fun init() {
        ...
        addConfigToggle(BUTTON_WIDTH, config.client.ref { it::showTradingStationTooltips })
        addConfigButton(BUTTON_WIDTH, config.client.ref { it::displayedFluidUnit }, FluidUnit.values().toList())
        addHeading(Text.translatable("gui.adorn.config.creative_inventory"), BUTTON_WIDTH)
        addConfigToggle(BUTTON_WIDTH, config.client.ref { it::showItemsInStandardGroups })
        addConfigButton(
            BUTTON_WIDTH,
            config.ref { it::groupItems },
